...
class MobileNetV3(nn.Module):
    def __init__(self, num_input_images, width_mult=1.):
        ...
        layers = [conv_3x3_bn(3 * num_input_images, input_channel, 2)]
...
class MobileEncoder(nn.Module):
    def __init__(self, pretrained, num_input_images=1):
        ...
        self.encoder = MobileNetV3(num_input_images)
        self.num_input_images = num_input_images
        if pretrained:
            state_dict = torch.load("./models/mobilenetV3/mobilenetv3-large-1cd25616.pth")
            filter_dict_enc = {k: v for k, v in state_dict.items() if k in self.encoder.state_dict()}
            if(num_input_images > 1):
                filter_dict_enc["features.0.0.weight"] = torch.cat([filter_dict_enc["features.0.0.weight"]] * num_input_images, 1) / num_input_images 
            self.encoder.load_state_dict(filter_dict_enc)
  
    def forward(self, input_image):
        self.features = []
        x = (input_image - 0.45) / 0.225
        #print(f'Num_input_image:{self.num_input_images} - Input_image:{x.shape} - First layer: {self.encoder.features[0]}')
        ...
        self.features.append(self.encoder.features[5:8](self.features[-1]))
        self.features.append(self.encoder.features[8:](self.features[-1]))
